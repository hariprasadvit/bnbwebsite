...
export default async function ServiceListing() {
  ...
  return (
    <div style={{ width: "100%" }}>
      <NewHeroSection data={{
        brandPrefix: data?.highlight || "Technology Solutions",
        brandName: data?.heading || data?.title || "That Drive Results",
        subtitle: data?.subheading || data?.description || "From custom applications to AI-powered systems, we build solutions that transform businesses and accelerate growth through innovative technology."
      }} />
      <OurServicesSection />
